  ...
  fetch() {
    try {
      ...
      while ((rowMatch = rowRegex.exec(html)) !== null) {
        ...
        // Filter out old cases - skip if time is > 6 hours ago
        const recordTime = new Date(time);
        const now = new Date();
        const sixHoursInMs = 6 * 60 * 60 * 1000; // 6 hours in milliseconds
        
        // Skip if time is invalid or older than 6 hours
        if (isNaN(recordTime.getTime()) || (now - recordTime) > sixHoursInMs) {
          continue;
        }
        
        // Also skip if time is in the future (> 1 day)
        if ((recordTime - now) > 86400000) {
          continue;
        }
